...
export const isNoSignal = (close, ema200, macd, signal, histogram) => {
    return !isPerfectBuySignal(close, ema200, macd, signal, histogram) && !isPerfectSellSignal(close, ema200, macd, signal, histogram)
        && !isWeakBuySignal(close, ema200, macd, signal, histogram) && !isWeakSellSignal(close, ema200, macd, signal, histogram)
        && !isDangerousBuySignal(close, ema200, macd, signal, histogram) && !isDangerousSellSignal(close, ema200, macd, signal, histogram)
}
...
export const formatMessage = (prices, timeframe) => {
    ...
    prices.forEach(price => {
        ...
        switch (timeframe) {
            case "H1":
                payload = [price.h1_close, price.h1_ema200, price.h1_macd, price.h1_signal, price.h1_histogram];
                break;
            case "M30":
                payload = [price.m30_close, price.m30_ema200, price.m30_macd, price.m30_signal, price.m30_histogram];
                break;
            case "M15":
                payload = [price.m15_close, price.m15_ema200, price.m15_macd, price.m15_signal, price.m15_histogram];
                break;
            case "M5":
                payload = [price.m5_close, price.m5_ema200, price.m5_macd, price.m5_signal, price.m5_histogram];
                break;
        }
